...
export default class ObjectSelector implements IIntersectionListener {
  ...
  private restoreFromPivotGroup(obj: THREE.Object3D) {
    obj.updateMatrixWorld(true);

    // 1. Store world matrix before detach
    const worldMatrix = obj.matrixWorld.clone();

    // 2. Remove from pivot group
    this.objects.add(obj);

    // 3. Apply world matrix directly
    obj.matrix.copy(worldMatrix);
    obj.matrix.decompose(obj.position, obj.quaternion, obj.scale);
    obj.updateMatrixWorld(true);
  }

  private exitTransformMode() {
    this.previousSelectedObjects.forEach(obj =>
      this.restoreFromPivotGroup(obj)
    );
    this.canvas.pivot_group.clear();
    this.canvas.mm.detach();
  ...
  public toggleSpaceModeUI() {
    this.spaceMode =
      this.spaceMode === spaceModes.World ? spaceModes.View : spaceModes.World;
    console.log(
      'Space Mode:',
      this.spaceMode === spaceModes.World ? 'World' : 'View'
    );
  ...
  onMouseDown(intersection: THREE.Intersection, pointer: MouseEvent) {
    ...
    if (
      ...
      //left click with translation
      this.interactive = false;
      this.isobjmoving = true;
      // this.temp_pos = this.previousSelectedObjects.map(obj => obj.position.clone());
      this.temp_rot = this.previousSelectedObjects.map(obj =>
        obj.rotation.clone()
      );
      this.eventdispatcher.dispatchEvent({
        type: 't_start',
        rotation: this.temp_rot,
        position: this.temp_pos
      });
    } else if (
      this.previousSelectedObject != undefined &&
      ...
      this.temp_pos = this.previousSelectedObjects.map(obj =>
        obj.position.clone()
      );
      this.temp_rot = this.previousSelectedObjects.map(obj =>
        obj.rotation.clone()
  ...
  onMouseUp(intersection: THREE.Intersection, pointer: MouseEvent) {
    ...
    if (this.isobjmoving) {
      ...
      if (
        Math.abs(this.canvas.pivot_group.position.x) +
          Math.abs(this.canvas.pivot_group.position.y) +
          Math.abs(this.canvas.pivot_group.position.z) >
        0.1
      ) {
      ...
      if (
        Math.abs(this.canvas.pivot_group.rotation.x) +
          Math.abs(this.canvas.pivot_group.rotation.y) +
          Math.abs(this.canvas.pivot_group.rotation.z) >
        0.1
      ) {
        let tempQuaternion = new THREE.Quaternion();
        let tempVector = new THREE.Vector3();
      ...
      this.eventdispatcher.dispatchEvent({
        ...
        rotation: this.temp_rot,
        position: this.temp_pos,
        object: this.previousSelectedObject
      });
    }
    this.interactive = true;
    this.state = modes.Cammode;
  ...
  async setkey(event: any, keydown_coordinate_: any) {
    if (this.canvas.rotate_counter > 0 || this.canvas.trans_counter > 0) {
      ...
      if (this.canvas.trans_counter > 0) this.canvas.trans_counter = 0;
      if (this.canvas.rotate_counter > 0) this.canvas.rotate_counter = 0;
    }
    ...
    switch (event.key.toLowerCase()) {
      case 't':
        if (
          ...
          const intersectInfo = this.raycaster.intersectObjects(
          ...
          const pivot = this.canvas.pivot_group;
          pivot.clear();

          // Compute pivot position: just in front of the selected object
          ...
          let pivotPosition: THREE.Vector3;

          if (intersectInfo) {
            pivotPosition = intersectInfo.point.clone();
          } else {
            const bbox = new THREE.Box3().setFromObject(
              this.previousSelectedObject
            );
            ...
            pivotPosition = sphere.center
              .clone()
              .add(cameraDir.multiplyScalar(-sphere.radius * 1.1));
          }
          ...
          if (this.spaceMode === spaceModes.View) {
            pivot.quaternion.copy(this.canvas.camera.quaternion);
          } else {
          ...
          // ✅ Reparent selected objects WITHOUT movement
          this.previousSelectedObjects.forEach(obj => {
            obj.updateMatrixWorld(true);
            ...
            // Apply relative transform safely
            obj.matrix.copy(localMatrix);
            obj.matrix.decompose(obj.position, obj.quaternion, obj.scale);
            obj.updateMatrixWorld(true);
          });

          // Show gizmo
          this.canvas.mm.attach(pivot);
      ...
      case 'r':
        if (
          !(window as any).istyping &&
          this.previousSelectedObjects.length != 0
        ) {
          this.setUpRaycaster(keydown_coordinate_);
          const intersectInfo = this.raycaster.intersectObjects(
            this.previousSelectedObjects,
          ...
          if (intersectInfo) {
            ...
            this.eventdispatcher.dispatchEvent({
              type: 'interactive',
              intersect: new THREE.Vector3(x, y, z)
            });
          }
        }
        break;
      ...
      case 'm':
        ...
        const Measure_intersect = this.raycaster.intersectObjects(
  ...
  onMouseDoubleclick(intersection: THREE.Intersection, pointer: MouseEvent) {
    ...
    if (intersection !== undefined) {
      ...
      if (pointer.shiftKey) {
        if (
          this.canvas.outlinePass.selectedObjects.indexOf(intersection.object) >
          -1
        )
          this.canvas.outlinePass.selectedObjects.splice(
            this.canvas.outlinePass.selectedObjects.indexOf(
              intersection.object
            ),
            1
          );
        else this.canvas.outlinePass.selectedObjects.push(intersection.object);
      } else {
        if (this.canvas.outlinePass.selectedObjects.length > 1)
          ...
          this.canvas.outlinePass.selectedObjects.length == 0
        )
          this.canvas.outlinePass.selectedObjects = [intersection.object];
        else this.canvas.outlinePass.selectedObjects = [];
      }
      ...
      let prev_names = '';
      let past_names = '';
      this.previousSelectedObjects.map(obj => {
        prev_names = prev_names + ',' + obj.name;
      });
      this.pastSelectedObjects.map(obj => {
      ...
      if (!pointer.shiftKey) {
        if (this.pastSelectedObject == undefined) {
          this.eventdispatcher.dispatchEvent({
            ...
            newObject: [
              this.previousSelectedObject.name,
              undefined,
              prev_names,
              past_names
            ]
  ...
  changeControls_total(
    ...
    try {
      ...
      for (let i = 0; i < this.previousSelectedObjects.length; i++) {
        currentRotation.push(
          new THREE.Quaternion().setFromEuler(
            this.previousSelectedObject.rotation
          )
        );
        targetRotation.push(
          new THREE.Quaternion().setFromEuler(newRotation[0])
        );
      }
      if (
        this.previousSelectedObjects[0].position.equals(newPosition[0]) &&
        currentRotation[0].equals(targetRotation[0])
      ) {
        return;
      }
      ...
      if (milliseconds <= 0) {
        ...
        this.changeTimeout = setInterval(() => {
          ...
          for (let i = 0; i < this.previousSelectedObjects.length; i++) {
            const nextPosition = this.previousSelectedObjects[i].position
              .clone()
              .lerp(newPosition[i], interPolateTime);
            const nextQuaternion = currentRotation[i]
              .clone()
              .slerp(targetRotation[i], interPolateTime);
            const nextRotation = new THREE.Euler().setFromQuaternion(
              nextQuaternion
            );
            this.previousSelectedObjects[i].position.copy(nextPosition);
            this.previousSelectedObjects[i].rotation.set(
              nextRotation.x,
              nextRotation.y,
              nextRotation.z
            );
  ...
  settransobj(position) {
    this.previousSelectedObject.position.set(
      position[0],
      position[1],
      position[2]
    );
  ...
  setrotateobj(rotation) {
    this.previousSelectedObject.rotation.set(
      rotation.x,
      rotation.y,
      rotation.z
    );
  ...
  setSelection(newSelection) {
    this.previousSelectedObject = this.objects.children.find(
      mesh => mesh.name === newSelection[0]
    ); // new
    this.pastSelectedObject = this.objects.children.find(
      mesh => mesh.name === newSelection[1]
    ); // old
    ...
    let temparr = [];
    let pasttemparr = [];
    if (newSelection[2] != '') temparr = newSelection[2].split(',');
    if (newSelection[3] != '') pasttemparr = newSelection[3].split(',');

    for (let i = 0; i < temparr.length; i++) {
      this.previousSelectedObjects.push(
        this.objects.children.find(mesh => mesh.name === temparr[i])
      );
    }

    for (let i = 0; i < pasttemparr.length; i++) {
      this.pastSelectedObjects.push(
        this.objects.children.find(mesh => mesh.name === pasttemparr[i])
      );
    }
