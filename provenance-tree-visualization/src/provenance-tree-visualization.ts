...
export class ProvenanceTreeVisualization {
  ...
  private renumberBranchPlans(): void {
    ...
    branchRoots.sort(
      (a, b) => a.metadata.branchnumber - b.metadata.branchnumber
    );
  ...
  public scaleToFit() {
    ...
    const trans_y =
      node_length > this.sizeY
        ? (this.currentHierarchyNodelength - node_max + margin) *
          yScale *
          maxScale
        : -20;
  ...
  public camerahide(): void {
    ...
    function find_noncameranode(c_trav: any) {
      ...
      if (traverser.label === "root") return traverser;
      do {
        const searchpattern = cam_test(traverser.label);
        if (!searchpattern) return traverser;
  ...
  public getFullsizeview(): void {
    ...
    const margin = 0;
    const node_length =
      (this.currentHierarchyMaxlength + 1 + margin) * yScale * maxScale;
  ...
  public removeNodesAndLinkChildren<T>(
    tree: IGroupedTreeNode<T>,
    condition: (node: IGroupedTreeNode<T>) => boolean
  ): IGroupedTreeNode<T> {
  ...
  public Grouping_hierarchy<T>(
    wraproot: IGroupedTreeNode<ProvenanceNode>
  ): d3.HierarchyNode<IGroupedTreeNode<ProvenanceNode>> {
    let hierarchyRoot = d3.hierarchy(wraproot);
    let allnodes = hierarchyRoot
      .descendants()
      .filter(
        (d: any) =>
          d.data.wrappedNodes[0].label !== "Root" && d.data.depth !== 1
      );
    ...
    allnodes.sort((a, b) => {
      return (
        a.data.wrappedNodes[0].metadata.H_value -
        b.data.wrappedNodes[0].metadata.H_value
      );
    });
    ...
    console.log(
      allnodes.map((node) => node.data.wrappedNodes[0].metadata.H_value)
    );
    console.log(
      allnodes.map((node) => node.data.wrappedNodes[0].metadata.O_group)
    );
    ...
    const uniqueValues = allnodes
      .filter(
        (node) =>
          node.parent!.data.wrappedNodes[0].metadata.O_group !=
            node.data.wrappedNodes[0].metadata.O_group &&
          node.parent!.data.children.length == 1 &&
          node.parent!.data.wrappedNodes[0].label != "Root"
      )
    ...
    groupslicenodes.sort((a, b) => {
      return b.depth - a.depth;
    });
    groupslicenodes.sort((a, b) => {
      return (
        a.data.wrappedNodes[0].metadata.branchnumber -
        b.data.wrappedNodes[0].metadata.branchnumber
      );
    });
    let Endnode = 0;
    let Startnode = 0;
    for (let i = 0; i < groupslicenodes.length; i++) {
      if (
        groupslicenodes[i].data.wrappedNodes[0].metadata.branchnumber !=
        groupslicenodes[i + 1]?.data.wrappedNodes[0].metadata.branchnumber
      ) {
        transferChildren(
          groupslicenodes[i].parent!.parent! as any,
          groupslicenodes[i].parent! as any,
          groupslicenodes[i] as any
        );
      } else if (
        groupslicenodes[i].depth - groupslicenodes[i + 1]?.depth ==
        1
      ) {
        Startnode = i;
        Endnode = i + 1;
        for (Endnode; Endnode < groupslicenodes.length; Endnode++) {
          if (
            groupslicenodes[Endnode].depth -
              groupslicenodes[Endnode + 1]?.depth !=
              1 ||
            groupslicenodes[Endnode].data.wrappedNodes[0].metadata
              .branchnumber !=
              groupslicenodes[Endnode + 1]?.data.wrappedNodes[0].metadata
                .branchnumber
          ) {
            break;
          }
        ...
        transferChildren_2(
          groupslicenodes[Endnode].parent,
          groupslicenodes[Endnode],
          groupslicenodes[Startnode]
        );
        i = Endnode;
      } else {
        transferChildren(
    ...
    const g_nodes = groupslicenodes.map(
      (node) => node.data.wrappedNodes[0].metadata.H_value
    );
    ...
    console.log(
      groupslicenodes.map((node) => node.data.wrappedNodes[0].metadata.H_value)
    );
    console.log(
      groupslicenodes.map((node) => node.data.wrappedNodes[0].metadata.O_group)
    );
  ...
  public update = () => {
    ...
    let camhideNodes = this.removeNodesAndLinkChildren(
      clonedWrappedRoot,
      (node) => node.camera === true
    );
    ...
    if (this.camera_show == true) {
      ...
      if (cam_test(this.traverser.graph.current.label)) {
        this.currentHierarchyNodelength = hierarchyRoot.path(
          this.keynode
        ).length;
        this.scaleToFit();
      }
    ...
    if (currentHierarchyNode === undefined) {
      this.traverser.toStateNode(
        hierarchyRoot.leaves()[0].data.wrappedNodes[0].id
      );
      this.traverser.toStateNode(this.traverser.graph.root.id);
      return;
    }
    this.currentHierarchyNodelength = hierarchyRoot.path(
      currentHierarchyNode as d3.HierarchyNode<IGroupedTreeNode<ProvenanceNode>>
    ).length;
    const tree = gratzl(
      hierarchyRoot,
      currentHierarchyNode as d3.HierarchyNode<IGroupedTreeNode<ProvenanceNode>>
    ...
    const treeNodes = tree
      .descendants()
      .filter((d: any) => d.data.wrappedNodes[0].metadata.option !== "merged");
    const treemaxwidth = tree
      .descendants()
      .map(function(item) {
        return item.x;
      })
    ...
    updateNodes.on("contextmenu", async (d: any) => {
      if (d.data.wrappedNodes.length != 1) return;
      await this.traverser.toStateNode(d.data.wrappedNodes[0].id, 0);
      this.traverser.graph.current = this.traverser.graph.getNode(
        d.data.wrappedNodes[0].id
      );
      d.data.wrappedNodes[0].metadata.bookmarked = !d.data.wrappedNodes[0]
        .metadata.bookmarked;
    ...
    updateNodes
      ...
      .filter((d: any) => {
        ...
        if ((this.traverser.graph.current as any).parent) {
          // 위에 뭐가 있는지 확인
          neighbourNode =
            (this.traverser.graph.current as any).parent ===
            d.data.wrappedNodes[0]
              ? true
              : neighbourNode; // 현 노드에 위가 있으면 네이버는 참
          d.data.wrappedNodes[0].metadata.neighbour = neighbourNode
            ? true
            : neighbourNode;
        }
        if ((this.traverser.graph.current as any).children.length !== 0) {
          for (const child of (this.traverser.graph.current as any).children) {
            neighbourNode = d.data.wrappedNodes.includes(child)
              ? true
              : neighbourNode;
            d.data.wrappedNodes[0].metadata.neighbour = neighbourNode
              ? true
    ...
    updateNodes
      .filter((d: any) => {
        if (d.data.wrappedNodes.length == 1) {
          const ref = d.data.wrappedNodes.includes(
          ...
          if (d.data.wrappedNodes.length == 1)
            ...
            d.data.wrappedNodes.forEach((node: any) => {
              if (node.metadata.bookmarked === true) {
                ...
                node.label.includes("Camera") ||
                node.label.includes("View")
              ) {
                colorScale.push("#60aa85");
              } else if (node.label.includes("SelectObject")) {
                colorScale.push("#b8852c");
              } else if (
                node.label.includes("TranslateObject") ||
                node.label.includes("RotateObject")
              ) {
            ...
            pieChartData.forEach((slice: any, index: number) => {
              ...
              pathElement.addEventListener("click", async () => {
                if (tempactive) tempactive.setAttribute("class", "");
              ...
              pathElement.addEventListener(
                ...
                async (event: MouseEvent) => {
                  ...
                  data[index].label.metadata.bookmarked = !data[index].label
                    .metadata.bookmarked;
                  data[index].bookmarked = !data[index].bookmarked;
                  if (!data[index].label.metadata.bookmarked) {
                    (window as any).slideDeckViz.onDelete(null);
                  } else {
                    (window as any).slideDeckViz.onAdd(
                      this.traverser.graph.current
                    );
                  }
                  this.update();
    ...
    updateNodes
      ...
      .attr(
        "class",
        (d: any) =>
          "circle-label renderer_" + getNodeRenderer(d.data.wrappedNodes[0])
      )
    ...
    updateNodes
      ...
      .attr(
        "class",
        (d: any) =>
          "circle-img renderer_" + getNodeRenderer(d.data.wrappedNodes[0])
      )
      .attr("visibility", (d: any) => (d.x === 0 ? "visible" : "hidden"));
    ...
    updateNodes.on("click", async (d, i) => {
      if (d.data.wrappedNodes.length > 1) {
        ...
        d.data.wrappedNodes[0].id !== this.traverser.graph.current.id
      ) {
    ...
    updateNodes
      ...
      .text((d) =>
        d.children
          ? ""
          : "Plan " + (d.data.wrappedNodes[0].metadata.branchnumber + 1)
      );

    const oldLinks = this.g.selectAll("path.link").data(
      tree
        .links()
        .filter(
          (d: any) => d.target.data.wrappedNodes[0].metadata.option !== "merged"
        ),
